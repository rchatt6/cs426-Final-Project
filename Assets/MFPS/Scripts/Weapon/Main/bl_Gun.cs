...
public class bl_Gun : bl_GunBase
{
    ...
    public void Initialized()
    {
        ...
        GunBob = m_Transform.root.GetComponentInChildren<bl_WeaponBob>();
    ...
    protected override void OnEnable()
    {
        ...
        {
            Debug.Log("active cross by: " + gameObject.name);
            Crosshair.Change(2);
        }
    ...
    protected override void OnDisable()
    {
        ...
        if (isAiming) { PlayerCamera.fieldOfView = DefaultFog; }
        StopAllCoroutines();
    ...
    void InputUpdate()
    {
        if (bl_GameData.Instance.isChating || !GManager.isGameStarted) return;
    ...
    void OnFire()
    {
        if (!GManager.isGameStarted) return;
    ...
    void OnLauncherNotAmmo()
    {
        ...
        foreach (GameObject go in OnAmmoLauncher)
        {
            ...
            if (bulletsLeft <= 0 && !isReloading)// if not have ammo
            {
                ...
                if (activeGrenade)
                {
                    PlayerSync.SetActiveGrenade(false);
                    activeGrenade = false;
                }
            }
            else
            {
                ...
                if (!activeGrenade)
                {
                    PlayerSync.SetActiveGrenade(true);
                    activeGrenade = true;
                }
            }
        }
        ...
        while (nextFireTime < Time.time)
        {
            ...
            if (Physics.Raycast(position, direction, out hit, Info.Range))
            {
                ...
                else if (hit.transform.CompareTag("AI"))
                {
                    ...
                    {
                        hit.transform.GetComponent<bl_AIShooterHealth>().DoDamage(Info.Damage, Info.Name, transform.position, bl_GameManager.LocalPlayerViewID, false, PhotonNetwork.LocalPlayer.GetPlayerTeam(), false);
            ...
            PlayerSync.IsFire(GunType.Knife, Vector3.zero);
            PlayFireAudio();
            nextFireTime += Info.FireRate;
            Kick();
            Shake();
    ...
    public IEnumerator ThrowGrenade(bool fastFire = false, bool useDelay = true)
    {
        ...
        PlayerSync.IsFireGrenade(spread, muzzlePoint.position, transform.parent.rotation, angular);
        PlayFireAudio();
        isFiring = false;
        //is Auto reload
    ...
    IEnumerator FireOneShot()
    {
        ...
        Crosshair.OnFire();
        PlayerSync.IsFire(Info.Type, hitPoint);
        if (Info.Type != GunType.Grenade)
        {
