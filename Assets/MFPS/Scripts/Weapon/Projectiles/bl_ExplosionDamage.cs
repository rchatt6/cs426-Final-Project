    ...
    private void DoDamage()
    {
        ...
        foreach (Collider c in colls)
        {
            if (c.CompareTag("AI"))
            {
                ...
                {
                    c.GetComponent<bl_AIShooterHealth>().DoDamage(damage, "Grenade", transform.position, AIViewID, isFromBot, t, false);
                }
                else if (c.GetComponent<bl_AIHitBox>() != null && !isNetwork)
                {
                    c.GetComponent<bl_AIHitBox>().DoDamage(damage, "Grenade", transform.position, AIViewID, isFromBot, t);
                }
            }
        }
    }
