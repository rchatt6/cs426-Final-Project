...
#pragma warning disable 0414
    ...
    public override void OnUpdate()
    {
        ...
        if (photonView != null && isMine == false)
        {
            ...
            HeatTarget.position = Vector3.Lerp(HeatTarget.position, HeadPos, Time.smoothDeltaTime * m_PositionModel.InterpolateLerpSpeed);
            ...
            {
                CurrentTPVGun();
                currentGunID = CurNetGun;
            }
        }
        else
        {
            m_PlayerAnimation.state = (int)Controller.State;//send the state of player local for remote animation
    ...
    void CurrentTPVGun(bool local = false)
    {
        if (gunManager == null)
            return;
        if (CurNetGun == currentGunID) return;
        ...
        for(int i = 0; i < NetworkGuns.Count; i++)
        {
            ...
            int currentID = (local) ? gunManager.GetCurrentWeapon().GunID : CurNetGun;
